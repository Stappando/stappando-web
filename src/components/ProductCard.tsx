...
export default function ProductCard({ product }: Props) {
  ...
  const isCircuito = product.tags?.some(t => t.id === 21993);
  ...
  return (
    <div className={`group rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full ${isCircuito ? 'bg-[#1a1a1a] border-2 border-[#b8973f]' : 'bg-white border border-gray-100 hover:border-gray-200'}`}>
      <Link href={`/prodotto/${product.slug}`} className="flex flex-col flex-1">
        ...
        <div className="flex-1 px-4 pt-3 pb-2 flex flex-col">
          ...
          <div className="flex items-baseline gap-2 mt-auto">
            <span className={`text-lg font-bold ${isCircuito ? 'text-[#b8973f]' : 'text-[#055667]'}`}>
              {formatPrice(product.price)} &euro;
            </span>
            {product.on_sale && product.regular_price && (
              <span className={`text-xs line-through ${isCircuito ? 'text-white/40' : 'text-gray-400'}`}>
                {formatPrice(product.regular_price)} &euro;
              </span>
            )}
          </div>
          <p className={`text-[10px] mt-1.5 flex items-center gap-1 ${isCircuito ? 'text-white/50' : 'text-gray-400'}`}>
          ...
          <p className={`text-[10px] font-medium ${isCircuito ? 'text-white/60' : 'text-gray-500'}`}>{vendorName || 'Stappando Enoteca'}</p>
        </div>
      </Link>
