...
export default function ProductCard({ product }: Props) {
  ...
  return (
    ...
    >
      {/* Image — tall, breathable */}
      <Link
        href={`/prodotto/${product.slug}`}
        className="relative aspect-[3/4] bg-[#f5f2ed] overflow-hidden"
      >
        ...
        {circuito && (
          <span className="absolute top-2.5 left-2.5 inline-flex items-center gap-1 bg-[#d9c39a] text-[#5a4200] text-[9px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full shadow-sm">
      ...
      <div className={`p-3.5 flex flex-col flex-1 ${circuito ? 'bg-[#1a1a1a]' : 'bg-white'}`}>
        {/* Vendor/Cantina — crema for circuito, petrolio for normal */}
        {produttore && (
          <p className="text-[10px] font-bold uppercase tracking-[0.05em] mb-1 line-clamp-1 text-[#d9c39a]">
            {produttore}
          </p>
        )}

        {/* Product name — white for circuito, dark for normal */}
        <Link href={`/prodotto/${product.slug}`}>
          <h3 className={`text-[13px] font-semibold leading-snug line-clamp-2 mb-2 ${circuito ? 'text-white' : 'text-[#1a1a1a]'}`}>
        ...
        <div className="flex items-baseline gap-1.5 flex-wrap mb-2">
          {product.on_sale && product.regular_price ? (
            <>
              <span className={`text-[12px] line-through ${circuito ? 'text-[#888]' : 'text-[#bbb]'}`}>{formatPrice(product.regular_price)}&euro;</span>
              {discount > 0 && (
                <span className="text-[10px] font-bold text-white bg-[#c0392b] px-2 py-0.5 rounded-full">-{discount}% SCONTO</span>
              )}
              <span className={`text-[18px] font-bold ${circuito ? 'text-white' : 'text-[#005667]'}`}>{formatPrice(product.price)}&euro;</span>
            </>
          ) : (
            <span className={`text-[18px] font-bold ${circuito ? 'text-white' : 'text-[#005667]'}`}>{formatPrice(product.price)}&euro;</span>
          )}
        ...
        <div className="flex items-center gap-1.5 mb-3">
          <svg className={`w-3 h-3 shrink-0 ${circuito ? 'text-[#777]' : 'text-[#999]'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <span className={`text-[10px] ${circuito ? 'text-[#777]' : 'text-[#999]'}`}>Venduto e spedito da {vendorName}</span>
        </div>
        ...
        {isOutOfStock ? (
          <button
            disabled
            className="w-full py-2.5 rounded-lg text-[12px] font-semibold bg-[#e8e8e8] text-[#aaa] cursor-not-allowed mt-auto"
          >
            Non disponibile
          </button>
        ) : (
          <button
            onClick={handleAdd}
            className={`w-full py-2.5 rounded-lg text-[12px] font-semibold transition-all mt-auto ${
              added
